...
def mpas_analysis(config, scriptDir):
    ...
    for c in tasks:
        ...
        for s, rs, es in zip(ts_year_sets, climo_year_sets, enso_year_sets):
            ...
            if c['subsection']:
                prefix = 'mpas_analysis_%s_ts_%04d-%04d_climo_%04d-%04d' % (c['subsection'],c['ts_year1'],c['ts_year2'],c['climo_year1'],c['climo_year2'])
            else:
                prefix = 'mpas_analysis_ts_%04d-%04d_climo_%04d-%04d' % (c['ts_year1'],c['ts_year2'],c['climo_year1'],c['climo_year2'])
            print(prefix)
            c['prefix'] = prefix
